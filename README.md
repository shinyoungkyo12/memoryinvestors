...
## 메모리 반도체 인덱스 (첫 화면 최상단)

ETF를 제외한 6종목(삼성전자·SK하이닉스·마이크론·샌디스크·씨게이트·웨스턴디지털)을 나스닥지수처럼 지수화한 자체 인덱스.

- **계산 방식**: 기준일 대비 각 종목 수익률을 동일가중 평균 → 기준일(2025-01)=1000. 수익률 기반이라 원/달러 환율·발행주식수 절대값 오차에 강건함.
- **과거 추이**: /api/index-history — 6종목 Yahoo 일봉을 공통 거래일(교집합)로 합성. 1시간 캐시.
- **실시간**: 구성종목 전일대비 등락률을 동일가중 평균해 과거 시계열 끝에 today 포인트로 연결.
- **구성종목/발행주식수**: lib/index-constituents.ts (시총가중 모드용 발행주식수는 "검증 필요" 주석 — 분기보고서 확인 후 갱신).
- **주의**: 한·미 장 시간대가 달라 한쪽만 열렸을 때는 해당 시장 종목만 실시간 반영됨.
