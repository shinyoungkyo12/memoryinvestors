...
export default function MemoryIndexChart({
  ...
  const periodChange = useMemo(() => {
    if (!data?.points.length || data.points.length < 2) return null;
    const f = data.points[0].value;
    const l = data.points[data.points.length - 1].value;
            ...
            {periodChange !== null && (
              <span
                className={`font-mono text-base font-bold tabular-nums ${
                  periodChange >= 0 ? "text-[var(--up)]" : "text-[var(--down)]"
                }`}
              >
